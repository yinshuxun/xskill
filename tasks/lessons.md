...
### 10. IGNORED_DIRS 忽略列表过度激进
* **Bug 场景**: 从 Project 导入 Skill 时，如果 Skill 位于 `.claude/skills/my-skill` 或 `.agent/skills/my-skill` 等目录下，导入到 Hub 的结果为空目录。
* **原因**: 在实现递归复制功能时，为了避免复制无关文件，定义了一个 `IGNORED_DIRS` 列表。但列表中包含了 `.claude`, `.agent` 等常见工具目录名。由于代码逻辑是检查路径中**任何部分**是否包含忽略项，导致源路径本身包含这些名称时，所有文件都被过滤掉了。
* **修复经验**: 谨慎定义忽略列表，只包含绝对需要忽略的系统级或构建级目录（如 `node_modules`, `.git`, `.DS_Store`）。对于工具特定的配置目录，不应在通用复制逻辑中全局忽略，除非确定它们不包含用户数据。

### 11. 前端搜索过滤的空值保护
* **Bug 场景**: 在 Hub 页面搜索框输入内容后清空，列表显示为空；或者搜索特定关键词时页面崩溃。
* **原因**: 数据源中存在 `description` 字段为 `undefined` 或 `null` 的 Skill。前端过滤逻辑直接调用 `.toLowerCase()` 导致运行时错误。同时，清空搜索框后过滤逻辑未及时短路，继续执行了错误的匹配逻辑。
* **修复经验**: 在处理可选字段的字符串操作时，始终使用 `(field || "")` 进行空值兜底。在搜索逻辑开始处，优先判断搜索词是否为空，如果是则直接返回全集，避免不必要的计算和潜在错误。

### 12. YAML/Markdown Frontmatter 解析的引号陷阱
* **Bug 场景**: `xskill sync --all` 命令报错找不到 Skill 目录。
* **原因**: 用户在 `SKILL.md` 的 Frontmatter 中使用了引号包裹字符串（如 `name: "my-skill"`）。简单的字符串切分解析将引号也作为名称的一部分（即名称变成了 `"my-skill"` 而非 `my-skill`），导致后续路径拼接错误。
* **修复经验**: 在手动解析 Frontmatter 或配置文件时，获取到的字符串值必须进行 `.trim_matches('"').trim_matches('\'')` 处理，以去除可能的首尾引号，确保获取到纯净的值。
