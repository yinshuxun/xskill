...
- [ ] **一键分享/发布**: 在脚手架写完 Skill 后，前端提供“Share to Internal”按钮，自动生成 JSON 配置片段并引导用户去内网提 PR。
...
### 3.2 环境变量管理
- [ ] **统一密钥面板**: 为每个安装的 Skill 提供独立的“环境变量配置表单” (API Keys 等)。
- [ ] **安全注入**: 同步至 IDE 时，将配置的变量一并注入到 `mcp.json` 的 `env` 字段中。
